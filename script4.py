...
switchdtbPattern = re.compile(r'(system.switch_cpus.dtb.misses).* ([0-9|\.]+)')
dcacheaccessPattern = re.compile(r'(system.cpu.dcache.overall_accesses).*(total).* ([0-9|\.]+)')
dcacheblockmshrPattern = re.compile(r'(system.cpu.dcache.blocked).*(no_mshrs).* ([0-9|\.]+)')
dcacheblocktargetPattern =re.compile(r'(system.cpu.dcache.blocked).*(no_targets).* ([0-9|\.]+)')
...
#           print threadlines
			...
			switchdtbmatch = switchdtbPattern.match(threadlines)
			dcacheaccessmatch = dcacheaccessPattern.match(threadlines)
			dcacheblockmshrmatch = dcacheblockmshrPattern.match(threadlines)
			dcacheblocktargetmatch = dcacheblocktargetPattern.match(threadlines)
            # dcachematch = dcachePattern.match(threadlines)
            # icachematch = icachePattern.match(threadlines)
            # l2match = l2cachePattern.match(threadlines)
            # tlbmatch = tlbPattern.match(threadlines)
...
#                        print threadlines
   ...
   #                              print "icachemshrmatch!!!"
			...
			if switchdtbmatch:
				print "switchdtbmatch!!!!"
				fpWrite.write("%s " %(switchdtbmatch.group(2)))
			if dcacheaccessmatch:
				print "dcacheaccessmatch!!!"
				fpWrite.write("%s " %(dcacheaccessmatch.group(3)))
			if dcacheblockmshrmatch:
				print "dcacheblockmshrmatch!!!"
				fpWrite.write("%s " %(dcacheblockmshrmatch.group(3)))
			if dcacheblocktargetmatch:
				print "dcacheblocktargetmatch!!!"
				fpWrite.write("%s " %(dcacheblocktargetmatch.group(3)))
